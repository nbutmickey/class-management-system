...
router.post("/applyBedRoomChief", function(req, res) {
  let { studentId, buildId, bedRoomId, repeat } = req.body.apply;
  if (repeat) {
    studentDao.updateApplyBedRoomChief(function(result) {
        if (result) {
            jsonUtils.jsonBack(
                res,
                true,
                null,
                "已提交寝室长申请表格，请耐心等待管理员回复！"
            );
        } else {
    ...
    studentDao.applyForBedRoomChief(
      function(result) {
        if (result) {
          jsonUtils.jsonBack(
            res,
